...
3. Copy the response from step 2 and update the `redirect_uris` in the JSON object to include the local development URL (ex: https://localhost:5191/console). Locate the `inbound_auth_config > config` section and modify the `redirect_uris` array:
...
5. Add the local development origin of the console application (https://localhost:5191) to the CORS allowed origins in `<THUNDER_HOME>/repository/conf/deployment.yaml`.
...
6. Run the Thunder Console application.
